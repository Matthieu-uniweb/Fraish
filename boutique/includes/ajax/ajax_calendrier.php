    ...
    if($compteur<$num_jour){
     ...
     }else
     {
		 ...
		 if(checkdate($mois,$num_jour_courant,$annee)){
		  ...
		  $num_jour_lettre=$num_jour_courant +  $num_jour -1;
			  if($num_jour_lettre == 6 || $num_jour_lettre == 13 || $num_jour_lettre == 20 || $num_jour_lettre == 27 || $num_jour_lettre == 34 || $num_jour_lettre == 41 || $num_jour_lettre==7 || $num_jour_lettre == 14 || $num_jour_lettre == 21 || $num_jour_lettre == 28 || $num_jour_lettre == 35 || $num_jour_lettre == 42){
			  $lien='<p>'.$num_jour_courant.'</p>';
			  $tab_jours[$compteur]=$lien;
              $num_jour_courant++;
			  }else
			  {
			  
		  $lien='<a href=\"javascript:actualiseJour('.$annee.','.$mois.','.$num_jour_courant.'), afficheCalendrier()\">'.$num_jour_courant.'</a>';
		  $tab_jours[$compteur]=$lien;
		  $num_jour_courant++;
		  }
		  }
