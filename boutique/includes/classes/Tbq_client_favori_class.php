...
class Tbq_client_favori
	...
	*/
	...
	/** 
	* @var int $ID_pointDeVente ID du point de vente où le menu sera préparé
	* @access  private 
	*/
	var $ID_pointDeVente;
	/**
	* @var int $ID_pointLivraison ID du point de livraison où le menu sera livré 
	*/
	...
	*/
	function initialiser()
		...
		if ( $this->ID>0 )
			{
			// initialisation de l'objet
			$requete = "SELECT * FROM boutique_obj_client_favori_v2 WHERE ID='".$this->ID."'";
	...
	*/
	function supprimer()
		{
		$requete = "DELETE FROM boutique_obj_client_favori_v2 WHERE ID='".$this->ID."'";
	...
	*/
	function enregistrer($valeurs)
		...
		if ($this->ID>0)
			...
			$requete = "UPDATE boutique_obj_client_favori_v2 SET ";
			...
			$requete = "INSERT INTO boutique_obj_client_favori_v2 SET ";
	...
	function lister()
		{
		$resultats = array();
		$requete = "SELECT ID FROM boutique_obj_client_favori_v2 ORDER BY ID_client ASC";
		$resSql = T_LAETIS_site::requeter($requete);
