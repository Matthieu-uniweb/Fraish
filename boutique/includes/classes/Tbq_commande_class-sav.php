...
class Tbq_commande
	...
	*/
	...
	var $reglement;
	/** 
	* @var string $statut Statut de la commande
	* @access  private 
	*/
	var $statut;
	...
	*/
	function enregistrer($valeurs)
		...
		$requete = "INSERT INTO boutique_obj_commande (dateCommande, dateReservation, ID_client,   
								ID_commande_fraish, ID_pointDeVente, ID_pointLivraison, soupe, plat, vinaigrette, boisson, boissonStat, dessert, pain, eau, supplement, taille, prix,
								commentaire, statut) 
								VALUES (NOW(), '".$valeurs['dateReservation']."', '".$valeurs['ID_client']."', 
								'".$resultats[0]['ID_commande_fraish']."', '".$valeurs['ID_pointDeVente']."',
	...
	*/
	function listerTousesCommandes($criteres)
		...
		if ($criteres['dateSeuleInsertion'])
	...
	*/
	...
	function genererEmailReservation($pageSource, $ID_client)
		...
		*/
		...
		$obj_mail->setFrom('service-reservations@fraish.fr');
	...
	*/
	...
	function getMontantHTSelonDate($date)
		{
		$ttc = Tbq_commande::getMontantTTCSelonDate($date);
		$ht = $ttc/(1+(5.5/100));
		return(round($ht,2));
		}
	function getMontantTVASelonDate($date)
		{
		return(Tbq_commande::getMontantTTCSelonDate($date)-Tbq_commande::getMontantHTSelonDate($date));
		}
