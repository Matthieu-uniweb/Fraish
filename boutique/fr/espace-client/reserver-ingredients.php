      ...
      <p><strong>Commande pour le : </strong>
		...
		for($i=1; $i<=31; $i++)
			...
			/*if(strlen($i)==1) {
				...
				echo $i;
			//}?>" <?php if($i == $jourSelect){?>selected="selected"<?php }?>><?php echo $i;?></option>
            <?php
			}
		?>
        </select>
...
$MaxDateEnregistre = Tbq_menujour::getMaxDateEnregistre();
$MaxDateEnregistre = explode('-',$MaxDateEnregistre);
$MaxDateEnregistre = $MaxDateEnregistre[0].$MaxDateEnregistre[1].$MaxDateEnregistre[2];

$dateDeReservation = explode('-',$dateReservation);
if(strlen($dateDeReservation[1])==1) {
	$dateDeReservation[1]='0'.$dateDeReservation[1];
}
if(strlen($dateDeReservation[0])==1) {
	$dateDeReservation[0]='0'.$dateDeReservation[0];
}
$dateDeReservation = $dateDeReservation[2].$dateDeReservation[1].$dateDeReservation[0];
$menuEstEnregistre = true;
if($dateDeReservation>$MaxDateEnregistre){ $menuEstEnregistre = false ; }
//echo $dateDeReservation.' -- '.$MaxDateEnregistre;
// Salade
...
<div id="supplements" style="float:left; width:500px; display:none;">
...
foreach ($listeFamille as $famille){
	...
	if ($sousFamilles[0]['libelleSousFamille']== ''){
		
		$supplementFam = Tbq_ingredient::getSupplements($famille['ID'],'ordreWeb');
		//print_r($supplementFam);
		if($supplementFam){
			echo '<strong>'.$famille['libelle'].'</strong><br/>';
			?>
            <table>
            <?php
			foreach($supplementFam as $supplement){
				...
				?>
				<tr>
                	<td style="width:190px;"><?php echo $supplement['libelle'];?></td>
                	<td style="width:100px;">quantit&eacute; <input type="text" name="sup[<?php echo $index;?>][qte]" value="0" class="inputQte"/></td>
                    <?php
                    if(count($prixOption)<=1){
                    ?>
                	<td><input type="hidden" name="sup[<?php echo $index;?>][nom]" value="<?php echo $supplement['ID'];?>" />
        <input type="hidden" name="sup[<?php echo $index;?>][prix]" value="<?php echo $prixOption[0]['prix']; ?>" />&nbsp;<?php echo $prixOption[0]['libelle'];?></td>
        			<?php } else { ?>Z
                    <td style="width:200px;" >
                      &nbsp;&nbsp;&nbsp;Taille 
                      <input type="hidden" name="sup[<?php echo $index;?>][nom]" value="<?php echo $supplement['ID'];?>" />
                      <select name="sup[<?php echo $index;?>][prix]">
                      	<?php
						foreach($prixOption as $option){
							echo '<option value="'.$option['prix'].'|'.$option['libelle'].'">'.$option['libelle'].' - '.$option['prix'].' euros</option>';
						}
						?>
                      </select>
                    </td>
		...
		foreach ($sousFamilles as $sFamille){
			...
			//print_r($supplementSsFamilles);
			//print_r($supplementSsFamilles);
			if ($supplementSsFamilles){
				echo '<br/><strong>'.$sFamille['libelleSousFamille'].'</strong><br/>';
				?>
            <table>
            <?php
			foreach($supplementSsFamilles as $supplement){
				...
				<tr>
                    ...
                    ?>
                	<td><input type="hidden" name="sup[<?php echo $index;?>][nom]" value="<?php echo $supplement['ID'];?>" />
        <input type="hidden" name="sup[<?php echo $index;?>][prix]" value="<?php $prixOption[0]['prix'] ?>" /></td>
        			<?php } else { ?>
                    <td style="width:200px;">
                      &nbsp;&nbsp;&nbsp;Taille 
                      <input type="hidden" name="sup[<?php echo $index;?>][nom]" value="<?php echo $supplement['ID'];?>" />
