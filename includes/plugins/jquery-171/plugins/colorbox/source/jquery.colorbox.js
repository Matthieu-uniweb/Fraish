...
(function ($, document, window) {
		...
		callback = function () {
            ...
            if (total > 1) { // handle grouping
                ...
                if (typeof settings.current === "string") {
					 ...
					 for(i=1;i<=total;i++){
						 if(index+1==i)
						 	liens+='<div class="link-actif">'+i+'</div>';
						 else
						 	liens+='<div class="link">'+i+'</div>';
					 }
					  $current.html(liens).show();
					 $('#cboxCurrent').children().click(function() {
						 var slideIndex = $('#cboxCurrent').children().index(this);
						 publicMethod.allerImage(slideIndex);
						 this.className = "link-actif"
					 });
	...
	publicMethod.allerImage = function (num) {
			index = num;
			publicMethod.load();
	};
